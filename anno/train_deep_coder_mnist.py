...
parser.add_argument("-n","--nb_iter",type=int, default=2, help="number of VAE iterations")
...
train_p = "/home/mihee/dev/project/robert_data/resized_MNIST/trainingSet/"
test_p = "/home/mihee/dev/project/robert_data/resized_MNIST/testSet/"
...
for i in range(0,1):
    ...
    for file in files:
        train_img_data.append(dir + "/" + file)
        label = np.zeros((10,2))
        label[:,1] = 1
        label[i] = [1,0]
        #one_label_onehot = np.zeros(10)
        #one_label_onehot[i] = 1
        #label_10digit = [[k] for k in one_label_onehot]
...
#train_img, validate_img,  train_label, validate_label= train_test_split(validate_img, validate_label, test_size=0.1)


train_data = {'img' : np.array(validate_img), 'lab': np.array(validate_label)}
...
inp_0_shape = X[0].shape[1:] # x[0] = one image from batch. thus, inp_0_shape = (240,160,1)
out_0_shape = Y[1].shape[1:] # y[1] = label. thus, out_0_shape = (10,)

#Tensor("input_1:0", shape=(?, 240, 160, 1), dtype=float32)
inp_0       = Input(shape=inp_0_shape) # 케라스의 텐서 선언 ???????? input shape만을 위해 X를 사용하고 데이터 사용 더이상 안함? 텐서는 사이즈만으로 뭘함?
emb, shape  = EE.networks.encoder(inp_0, norm=1) # conv넷을 여러번 씌워준 결과 network = emb /  shape = [15,10,16]


from numpy import prod
from keras.layers import Dropout

n_feat = prod(shape) # 2400
emb = Dropout(0.5)(emb)
# < ================== encoder =================================================
...
z_log_sigma = Dense(latent_dim)(emb) # z_mean & z_sigma's shape = (n,2000)
...
D1 = Dense(latent_dim, activation='relu') # latent_dim이 ouputspace의 dim인 fully connected layer. input = 10*2000, output = 10*2000, W = (2000, 2000), b=(2000,1)
D2 = Dense(n_feat, activation='sigmoid')  # (n_feat  = conv 넷 결과 shape들의 곱)이 ouputspace의 dim. input = 10*2000, output = 10*2400, W = (2000, 24000), b = (2400,1)

#Tensor("Relu_4:0", shape=(10, 2000), dtype=float32)
h_decoded = D1(z) # (10,2000) tensor. latent space에서 샘플링한 z를 인풋으로하여 아웃풋도 latent space인 fullyconnected layer. 근데 relu함

#Tensor("Sigmoid:0", shape=(10, 2400), dtype=float32)

...
for subject in os.listdir(original_frame_path):
    ...
    lab_au = np.array(intensities_for_one_au)
    ...
    min_len = np.min([len(detected_frame_idx), len(lab_au)])

    total_on_intst_idx = [i for i in detected_frame_idx[:min_len] if lab_au[i] == 1]
    print(total_on_intst_idx)
    print(">>>>>>>>>>>", len(total_on_intst_idx))
    total_off_intst_idx = [i for i in detected_frame_idx[:min_len] if lab_au[i] == 0]
    ...
    kshot_path = "/home/ml1323/project/robert_data/DISFA/kshot_rest/" + au + "/"

...
class MAML:
    ...
    def construct_model(self, input_tensors=None):
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(inp, reuse=True):
                ...
                this_weight = {'w1': this_w, 'b1': this_b, 'w2': weights['w2'], 'b2': weights['b2'], 'w3': weights['w3'], 'b3': weights['b3']}
                ...
                def predict_other_au(i, input, label):
                    ...
                    other_weight = {'w1': other_w, 'b1': other_b, 'w2': weights['w2'], 'b2': weights['b2'], 'w3': weights['w3'], 'b3': weights['b3']}
                ...
                task_lossb.append(task_ce_lossb + self.LAMBDA2 * task_co_lossb)
                ...
                for j in range(num_updates - 1):
                    ...
                    task_co_lossb = tf.reduce_sum(task_co_lossb, 0) / self.total_num_au
                    task_lossb.append(task_ce_lossb + self.LAMBDA2 * task_co_lossb)
                #task_lossb = tf.reduce_sum(task_lossb)
                task_lossb = task_lossb[-1]
                ...
                if FLAGS.adaptation:
                    ...
                    task_output = [fast_weights['w1'], fast_weights['b1'], task_ce_lossb, task_co_lossb, task_lossb,
                                   predb_this_au]
                return task_output
       ...
       # update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope='model')
        if FLAGS.opti.startswith('adadelta'):
            ...
            #self.train_op = tf.group(self.metatrain_op0, update_ops)
    ...
    def forward_fc(self, inp, weights):
        inp = tf.nn.relu(inp)
        mean = normalize(tf.matmul(inp, weights['w2']) + weights['b2'], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, scope='model')
        sigma = normalize(tf.matmul(inp, weights['w3']) + weights['b3'], activation=tf.nn.relu, reuse=tf.AUTO_REUSE, scope='model')
        sample = mean + sigma * tf.random_normal(tf.shape(mean), 0, 1, dtype=tf.float32)
        z = tf.reduce_sum(weights['w1'][None, ::] * sample[:,:,None,None], 1) + weights['b1'][None, ::]
    ...
    def getWeightVar(self):
        ...
        weights['w3'] = tf.get_variable('w3', [self.dim_input, self.weight_dim[1]], initializer=fc_initializer)
        weights['b3'] = tf.get_variable('b3', [self.weight_dim[1]], initializer=tf.zeros_initializer())

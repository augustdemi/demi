...
def get_kshot_from_img_path(path, seed, nb_samples=None, validate=True):
    subject = int(path[-1])
    print("============================================")
    print(">>>>>>>>>>>>>subject: ", subject)
    # random seed는 subject에 따라서만 다르도록. 즉, 한 subject내에서는 k가 증가해도 계속 동일한 seed인것.
...
def get_kshot_feature(kshot_path, feat_path, seed, nb_samples=None, validate=True):
    ...
    for label in labels:
        ...
        for line in lines:
            line = line.split(',')
            feat_data.update({line[1], line[2:]})  # key = frame, value = feature vector
        feature_list = []
        for i in img_path_list:
            frame = img_path_list[i].split('/')[-1].split('.')[0]
            feature_list.append(feat_data[frame])
        num_existing_samples.append(len(feature_list))
